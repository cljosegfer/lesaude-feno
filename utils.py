...
def plot_log(model_label, log_trn, log_val = None, epoch = None):
    plt.figure();
    plt.plot(log_trn);
    if log_val != None:
...
def export(model, model_label, epoch):
    if epoch != None:
        ...
        torch.save(model, 'output/{}/partial_{}.pt'.format(model_label, epoch))
    else:
        print('exporting last model'.format(epoch))
        torch.save(model, 'output/{}/{}.pt'.format(model_label, model_label, epoch))
