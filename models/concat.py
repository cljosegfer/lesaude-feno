...
class ResnetConcat(nn.Module):
    def __init__(self, n_classes, num_blocks, channels, feno_embedding, dropout_rate=0.2):
        super().__init__()
        ...
        self.linear = nn.Linear(960 + feno_embedding, n_classes)
        self.feno_encoder = nn.Linear(2, feno_embedding)

    def forward(self, x, age, ismale):
        x = self.input_layer(x)
        x = self.residual_blocks(x)
        x = self.flatten(x)
        # logits = self.linear(x)

        feno = self.feno_encoder(torch.stack((age, ismale), dim = 1))
        logits = self.linear(torch.cat((x, feno), dim = 1))
