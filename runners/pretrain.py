...
def pretrain_loss(output, age, ismale):
    return F.mse_loss(output[:, 0], age) + F.binary_cross_entropy_with_logits(output[:, 1], ismale)
...
class Runner():
    def __init__(self, device, model, database, model_label = 'baseline'):
        ...
        self.trn_ds = CODEsplit(self.database, self.database.trn_idx_dict)
        self.val_ds = CODEsplit(self.database, self.database.val_idx_dict)
        self.tst_ds = CODEsplit(self.database, self.database.tst_idx_dict)
    ...
    def _train_loop(self, loader, optimizer, criterion):
        ...
        for batch in tqdm(loader):
            ...
            loss = criterion(output, age, ismale)
